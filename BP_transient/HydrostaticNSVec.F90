...
CONTAINS
  ...
  FUNCTION EffectiveViscosityVec( ngp, ntot, BasisVec, dBasisdxVec, Element, NodalVelo, &
    ...
    TYPE(ValueHandle_t), SAVE :: Visc_h, ViscModel_h, ViscExp_h, ViscCritical_h, &
        ...
        ViscTemp_h, ViscFloor_h
    REAL(KIND=dp) :: R, vgrad(2,3), NewtonRelax
    REAL(KIND=dp) :: c1, c2, c3, c4, Tlimit, ArrheniusFactor, A1, A2, Q1, Q2, ViscCond, ViscMin
    LOGICAL, SAVE :: ConstantVisc = .FALSE., Visited = .FALSE., DoNewton, GotRelax = .FALSE.
    LOGICAL, SAVE :: HaveViscFloor = .FALSE.
    ...
    IF(InitHandles ) THEN
      ...
      IF( ListGetElementSomewhere( ViscModel_h) ) THEN
        ...
        ! Optional floor on the effective viscosity (high-strain-rate end)
        CALL ListInitElementKeyword( ViscFloor_h,'Material','Minimum Viscosity')
        HaveViscFloor = .NOT. ViscFloor_h % NotPresentAnywhere
    ...
    IF(SaveShear) THEN
      IF( ShearVar % TYPE == Variable_on_nodes ) THEN
        ! Nodal accumulation needs DetJVec; only the bulk-assembly call has it.
    ...
    ! Optional user-specified floor on the effective viscosity.
    ! Guards against the Glen shear-thinning runaway (strain rate explodes ->
    ! viscosity -> 0) in under-resolved / over-steep elements, where the
    ! native BP velocity would otherwise diverge. Applied additively
    ! (eta_eff = eta_glen + eta_min) rather than as a hard max(): this is
    ! smooth, so it avoids the iteration-to-iteration switching (chatter)
    ! of a hard clamp, and leaves the strain-rate derivative untouched
    ! (d eta_min / d strain = 0), keeping the Newton Jacobian consistent.
    ! eta_min should be set well below the smallest physical viscosity so
    ! the everywhere-offset is negligible (a few % at most in the lowest
    ! real regions) and only bites where eta_glen has collapsed.
    IF( HaveViscFloor ) THEN
      ViscMin = ListGetElementReal( ViscFloor_h, Element=Element, Found=Found )
      IF( Found ) ViscVec(1:ngp) = ViscVec(1:ngp) + ViscMin
    END IF
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE PopulateDerivedFields()
    ...
    IF( dofs == 1 ) THEN
      ...
      END DO
    ELSE IF( dofs > 1 ) THEN
      ! Vector field (e.g. Flow Solution): single source of truth, shared with
      ! the per-nonlinear-iteration refresh in the solver's nonlinear loop.
      CALL UpdateVeloVectorHorizontal()
    END IF
    ...
    DO i=1,Mesh % NumberOfNodes                   
      ...
      IF(dofs > 0 .AND. DownPointer(i) == i) THEN
        ...
        DO WHILE(.TRUE.)
          ...
          IF(k1>0 .AND. k2>0 .AND. j1>0 .AND. j2>0) THEN
      ...
      DO i=1,Mesh % NumberOfNodes                   
        ...
        IF(DownPointer(i)==i) THEN
          ...
          IF (LimitVelocity) THEN
            vabs = SQRT(Vave(1)**2 + Vave(2)**2)
            ibits = transfer(vabs, 0_INT64)
            IF (iand(ibits, int(Z'7FF0000000000000', INT64)) == int(Z'7FF0000000000000', INT64)) THEN
              Vave = 0.0_dp
            ELSE IF (vabs > vlimit) THEN
              Vave = Vave * (vlimit / vabs)
            END IF
          END IF
    ...
    ! Verify HOVIvel is finite — NaN or Inf here may cause downstream problems
    IF (ASSOCIATED(VarXYAve)) THEN
      DO idx = 1, SIZE(VarXYAve % Values)
        ibits = transfer(VarXYAve % Values(idx), 0_INT64)
        IF (iand(ibits, int(Z'7FF0000000000000', INT64)) == int(Z'7FF0000000000000', INT64)) &
          CALL Fatal('HydrostaticNSVec','NaN or Inf in HOVIvel after PopulateDerivedFields')
      END DO
    END IF
  ...
  SUBROUTINE UpdateVeloVectorHorizontal()
    ...
    IMPLICIT NONE
    TYPE(Variable_t), POINTER :: VarXY, VarFull
    TYPE(Solver_t), POINTER :: pSolver
    TYPE(Mesh_t), POINTER :: Mesh
    CHARACTER(LEN=MAX_NAME_LEN) :: str
    LOGICAL :: Found
    INTEGER :: i, jx, jf, vdofs

    pSolver => CurrentModel % Solver
    Mesh => pSolver % Mesh

    ! Primary variable is the 2-dof horizontal velocity; bail out if that is not the case.
    VarXY => pSolver % Variable
    IF( .NOT. ASSOCIATED(VarXY) ) RETURN
    IF( VarXY % Dofs /= 2 ) RETURN

    str = ListGetString( pSolver % Values, 'Velocity Vector Name', Found )
    IF( .NOT. Found ) str = 'Flow Solution'
    VarFull => VariableGet( Mesh % Variables, str, ThisOnly = .TRUE. )
    IF( .NOT. ASSOCIATED(VarFull) ) RETURN   ! nothing to refresh if the field is absent
    ...
    DO i = 1, Mesh % NumberOfNodes
      jx = VarXY % Perm(i)
      jf = VarFull % Perm(i)
      IF( jx < 1 .OR. jf < 1 ) CYCLE
      VarFull % Values(vdofs*(jf-1)+1) = VarXY % Values(2*jx-1)
      VarFull % Values(vdofs*(jf-1)+2) = VarXY % Values(2*jx)
    END DO

  END SUBROUTINE UpdateVeloVectorHorizontal
...
100 CONTINUE
    ...
    ! Make the derived velocity-vector field (e.g. Flow Solution) carry the current
    ! horizontal velocity before assembling boundary terms, so a velocity-dependent
    ! basal sliding USF reads this iteration's velocity rather than the once-per-SS
    ! value from PopulateDerivedFields (which otherwise lags and breaks SS convergence).
    CALL UpdateVeloVectorHorizontal()
    ...
    DO idx = 1, SIZE(Solver % Variable % Values)
      ibits = transfer(Solver % Variable % Values(idx), 0_INT64)
      IF (iand(ibits, int(Z'7FF0000000000000', INT64)) == int(Z'7FF0000000000000', INT64)) &
        CALL Fatal(Caller,'NaN or Inf detected in velocity solution')
    END DO
